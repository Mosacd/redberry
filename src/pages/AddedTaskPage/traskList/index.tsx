...
const TaskList:React.FC<{appliedDepartments:string[], appliedPriorities: string[]; appliedEmployee: string | null; }>
 = ({appliedDepartments, appliedPriorities, appliedEmployee}) =>{
    ...
    const {data:tasks = [],} = useGetTasks();
      ...
      const filteredItems = tasks.filter((task) => {
        const matchesDepartment = appliedDepartments.length === 0 || appliedDepartments.includes(task.department.name);
        const matchesPriority = appliedPriorities.length === 0 || appliedPriorities.includes(task.priority.name);
        const matchesEmployee = !appliedEmployee || task.employee.name === appliedEmployee;
    ...
    <div className='w-[1680px] flex justify-center gap-[52px] mt-[79px]'>
        ...
        const filteredTasks = filteredItems?.filter((task) => task.status.id === status.id);
          ...
          return(   <div className='flex flex-col gap-[30px]' key={status.id}>
        ...
        >
            <div className='flex justify-between w-[341px] items-center'>
              <div className='flex gap-[10px] items-center'>
                <div className={`flex gap-[4px] items-center w-[86px] h-[26px] border-[0.5px] rounded-[5px] p-[4px] text-[12px] leading-[150%] font-[600]`} style={{
              borderColor: colorsPriority[task.priority.id-1],
              color: colorsPriority[task.priority.id-1],
            }}><img src={task.priority.icon} alt="" /> {task.priority.name}</div>
                <div className='w-[88px] h-[24px] text-[#FFFFFF] py-[5px] px-[9px] rounded-[15px] text-[12px] truncate'
                style={{
          ...
          <div className='flex justify-between w-[341px]'>
            <img src={task.employee.avatar} alt="" className='w-[31px] h-[31px] rounded-full' />
