...
const CustomSelect: React.FC<{
  setAppliedDepartments: Dispatch<SetStateAction<string[]>>;
  ...
  const [selectedDepartments, setSelectedDepartments] =
    useState<string[]>(appliedDepartments);
  const [selectedPriorities, setSelectedPriorities] =
    useState<string[]>(appliedPriorities);
  ...
  const handleTagClick = (tag: string) => {
    if (appliedDepartments.includes(tag)) {
      ...
      setAppliedPriorities((prev) => prev.filter((pri) => pri !== tag));
  ...
  return (
    <div className="px-[2px] flex w-full justify-between  max-w-[688px] gap-[45px] border-[1px] border-[#DEE2E6] rounded-[10px] relative">
      ...
      {selectedOption != null && (
        <form
          className={`flex flex-col z-50 w-full pt-[40px] px-[30px] pb-[20px] absolute border-[0.5px] border-[#8338EC] top-[55px] rounded-[10px] max-h-[275px] gap-[25px] bg-[#FFFF]`}
          onClick={(e) => e.stopPropagation()}
        >
          <div
            className="flex flex-col gap-[22px] items-start  *:flex *:gap-[15px] overflow-auto *:items-center **:cursor-pointer"
            style={{
              scrollbarWidth: "thin",
          ...
          >
                ...
                return (
                  <label htmlFor={department.name}>
                    <Checkbox
                      ...
                      onCheckedChange={() =>
                        handleDepartmentChange(department.name)
                      }
                    />
                    {department.name}
                  </label>
            ...
            {selectedOption == "პრიორიტეტი" &&
              priorities.map((priority) => {
                return (
                  <label htmlFor={priority.name} key={priority.id}>
                    <Checkbox
                      id={priority.name}
          ...
          <div className="w-full flex justify-end">
            <button
              ...
              className="py-[9px] px-[20px] bg-[#8338EC] hover:bg-[#B588F4] transition duration-200 rounded-[20px] text-[#FFFFFF] w-[155px] cursor-pointer"
            >
              არჩევა
            </button>
          </div>
      ...
      <div className="flex mt-[69px] gap-[8px] absolute w-[1680px] *:cursor-pointer">
        {[
          ...
          ...(appliedEmployee
            ? [
                appliedEmployee +
                  ` ${
                    employees.find(
                      (employee) => employee.name === appliedEmployee
                    )?.surname
                  }`,
              ]
            : []),
          ...
          return (
            <button
              className="flex h-[29px] gap-[4px] px-[10px] py-[6px]  border-[1px] bg-[#FFFFFF] border-[#CED4DA] rounded-[43px]"
              onClick={() => handleTagClick(tag)}
            >
              <span
                className={`text-[14px] text-[#343A40] text-center
                        ${
                          appliedDepartments.length > 3 &&
                          appliedDepartments.includes(tag)
        ...
        {(appliedDepartments.length > 0 ||
          ...
          (appliedEmployee != null && appliedEmployee != "")) && (
          <button
            onClick={handleAllTagClick}
            className="text-[14px] text-center text-[#343A40] px-[10px] py-[6px] rounded-[43px] bg-[#FFFFFF]"
          >
            გასუფთავება
          </button>
        )}
